class LineCompare:
  ...
  def color_diff_for_line(self, line, startIndex, length, color):
    lineLeft, lineMid, lineRight = self.split_line_at_three(
  ...
  def lineCompare(self, lineNo, line1, line2):
    ...
    for i in range(minLength):
      if line1[i] != line2[i]:
        ...
        line1Ans = self.color_diff_for_line(line1, startIndex, len(diff),
                                            'magenta')
        line2Ans = self.color_diff_for_line(line2, startIndex, len(diff),
                                            'magenta')
        diff = ''

    if minLength < maxLength and maxLength == len(line1):
      line1Ans = self.color_diff_for_line(line1, minLength, maxLength,
                                          'magenta')
    elif minLength < maxLength and maxLength == len(line2):
      line2Ans = self.color_diff_for_line(line2, minLength, maxLength,
                                          'magenta')
